...
class UserStatus:
    ...
    async def join_game(
        ...
        if await pipe.json().get(user_id):
            ...
            await pipe.json().set(
                user_id,
                ".",
                asdict(UserStatus.UserState(current_game=game_id, queued_games=[])),
            )
    ...
    async def check_users_are_ready(game_id: GameId, user_ids: List[UserId]) -> bool:
        for user_id in user_ids:
            ...
            if user_status:
                if (
                    user_status.current_game != game_id
                    and user_status.current_game != None
                ):
                    return False
        return True
            ...
            if current_status.current_game == game_id:
                if len(current_status.queued_games) > 0:
                    await pipe.json().set(
                        user_id,
                        ".current_game",
                        await pipe.json().arrpop(str(user_id), ".queued_games", 0),
                    )
                else:
                ...
                if (
                    index := await pipe.json().arrindex(
                        str(user_id), ".queued_games", game_id
                    )
                ) > -1:
