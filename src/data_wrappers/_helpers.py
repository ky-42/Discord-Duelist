...
def pipeline_watch(redis_pool: redis_sync.Redis, watch_param_name: str, value_not_found_excepton: Type[Exception] = ValueError):
    def decorator(func):
                ...
                async with redis_pool.pipeline() as pipe:
                    ...
                    while await pipe.exists(watch_data):
                        try:
                            ...
                            continue
                    raise value_not_found_excepton(f'Value of {watch_param_name} not found')
            else:
